...
var list = function() {
    var list = function () {
        ...
        f.iteration = function () {
            this.delete = function (data) {//deletes first node to update start pointer
                var current = this.start;
                var prev = this.start;
                while (current !== null) {
                    if (data === current.data) {
                        if (current === this.start) {
                            return this.start = current.next;
                        }
                        if (current == this.end) this.end = prev;
...
//var test = new list();  //LinkedList list = new LinkedList();
//test.cons ('G') //list.add();
//test.cons ('T')
//document.writeln(test());
//document.writeln(test.iteration());

document.writeln(l3.iteration());
//document.writeln(l3.iteration());
