...
print sorted([[i[1], i[2], i[6], i[7]] for i in s_emp[1::] if i[7] > 1500 and i[9] > 40], key=lambda x: x[3],
             reverse=True)

print "\nselect last_name, first_name, title, salary, name from s_emp e join s_dept d on(e.dept_id = d.id):"
print [[i[1], i[2], i[6], i[7], j[1]] for i in s_emp[1::] for j in s_dept[1::] if int(i[9]) == int(j[0])]

print "\nselect dept_id, avg(salary) from s_emp group by dept_id order by dept_id:"
for department in sorted({d[9] for d in s_emp[1::]}): print (lambda deptno, avgSal: (deptno, avgSal))(department,
    (lambda l: round(sum(l) / len(l), 2))(map(float, [ e[7] for e in s_emp[1::] if e[9] == department ])))



print "\nselect dept_id, avg(salary) from s_emp group by dept_id having avg(salary) < 1500:"
for department in sorted({d[9] for d in s_emp[1::]}): print (lambda deptno, avgSal: (deptno, avgSal) if avgSal < 1500 else '')(department,
    (lambda l: round(sum(l) / len(l), 2))(map(float, [ e[7] for e in s_emp[1::] if e[9] == department ])))